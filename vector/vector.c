...
static void *create_element(void *el){
  ...
  void *nw_el = NULL; nw_el = malloc(sizeof(void*));
  __CONDITION_ERROR((nw_el != NULL), "create_element (static function to copy *el)", "Malloc error");
...
void *vector_element(Vector v, unsigned int idx){
  ...
  __CONDITION_ERROR((idx < vector_size(v)), "vector_element", "Overflow of index");
...
unsigned int vector_search(Vector v, void *el, int(*cmp)(const void *a, const void *b)){
  ...
  __CONDITION_ERROR((idx == vector_size(v) || cmp(v->array[idx], el) == 0), "vector_search", "Bad search");
  return idx;
}
...
unsigned int vector_sorted_search(Vector v, void *el, int(*cmp)(const void *a, const void *b)){
  ...
  __CONDITION_ERROR((idx == vector_size(v) || cmp(v->array[idx], el) == 0), "vector_sorted_search", "Bad search");
  return idx;
}
...
void vector_assign(Vector v, void *el, unsigned int idx){
  ...
  __CONDITION_ERROR((idx < vector_size(v)), "vector_assign", "Overflow of index");

  void *nw_el = create_element(el);
  free(v->array[idx]); v->array[idx] = NULL;
  v->array[idx] = nw_el;
}
...
void vector_pop_back(Vector v){
  ...
  __CONDITION_ERROR((!vector_is_empty(v)), "vector_pop_back", "Vector is empty");

  free(v->array[v->size-1]); v->array[v->size-1] = NULL;
  v->size--;
}
...
void vector_erase(Vector v, unsigned int idx){
  ...
  __CONDITION_ERROR((idx < vector_size(v)), "vector_erase", "Overflow of index");
...
static void merge_sort(Vector v, int(*cmp)(const void *a, const void *b), unsigned int left, unsigned int right){
  if(left < right){
    unsigned int middle = left+(right-left)/2;
    merge_sort(v, cmp, left, middle);
    merge_sort(v, cmp, middle+1, right);

    Vector aux = NULL; aux = vector_initialize(aux);
    unsigned int i = left, j = middle+1;
    while(i <= middle && j <= right){
      if(cmp(v->array[i], v->array[j]) <= 0){
        vector_push_back(aux, v->array[i]); i++;
      }else{
        vector_push_back(aux, v->array[j]); j++;
      }
    }
    while(i <= middle){
      vector_push_back(aux, v->array[i]); i++;
    ...
    __CONDITION_ERROR((i == middle+1 && j == right+1), "merge_sort", "Bad sorting");
  }
}
